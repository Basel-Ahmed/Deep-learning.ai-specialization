...
def initialize_parameters_deep(layer_dims):
    ...
    for l in range(1, L):
        parameters['W' + str(l)] = np.random.randn(layer_dims[l], layer_dims[l-1]) * np.sqrt(6/(layer_dims[l-1] + layer_dims[l]) ) #*0.01
...
def compute_cost(AL, Y, parameters, lambd):
    ...
    for i in range (1,int ( ( len(parameters)/2 )+1 ) ):
        l.append( np.sum( np.square( parameters[ "W"+str(i) ]  )  ) )
    
    l=[]
    #regularization=regularization*( lambd / (2*m) )
    # Compute loss from aL and y.
    cost = (1./m) * (-np.dot(Y,np.log(AL).T) - np.dot(1-Y, np.log(1-AL).T))
    cost=cost+ ( lambd / (2*m) )*sum(l)
...
def linear_activation_backward_with_drop_out(dA, cache, activation,lambd,keep_prob):
    """
    ...
    db -- Gradient of the cost with respect to b (current layer l), same shape as b
    """
    linear_cache, activation_cache,drop_out_cache = cache
    
    if activation == "relu":
        dZ = relu_backward(dA, activation_cache)
        dA_prev, dW, db = linear_backward(dZ, linear_cache,lambd)
        dA_prev = np.multiply(dA_prev,drop_out_cache)
...
def L_model_backward(AL, Y, caches,lambd,keep_probs):
    """
    ...
    grads["dA" + str(L-1)], grads["dW" + str(L)], grads["db" + str(L)] = linear_activation_backward_with_drop_out(dAL, current_cache,  "sigmoid",lambd,keep_probs[L-1])
    
    for l in reversed(range(L-1)):
        # lth layer: (RELU -> LINEAR) gradients.
        linear_cache = caches[l][0]
        activation_cache = caches[l][1]
        drop_out_cache= caches[l-1][2]
        current_cache=(linear_cache,activation_cache,drop_out_cache)
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward_with_drop_out(grads["dA" + str(l + 1)], current_cache,  "relu",lambd,keep_probs[l])
...
def predict(X, y, parameters):
    ...
    # Forward propagation
    probas, caches = L_model_forward(X, parameters,[1,1,1,1])
...
def vector_to_dictionary(theta):
    ...
    """
    parameters = {}
    parameters["W1"] = theta[:245760].reshape((20,-1))
